...
@section('title', 'Newllter')
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Newllter <span class="total-item">(0)</span></h5>
        <div class="btn-group" role="group">
            <a href="{{ route('user.contact.index') }}" class="btn btn-secondary me-1">
                Liên hệ
            </a>
            <a href="{{ route('user.newllter.index') }}" class="btn btn-primary me-1">
                Newllter
            </a>
...
@push('js')
    <script>
        const routeList = "{{ route('user.newllter.list') }}";

        filterTable();

        function downFile() {
            var data = $('#form-filter').serialize();
            const url = "{{ route('user.newllter.list') }}?" + data + "&export=1";
            window.open(url);
        }
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.newllter.delete') }}");
        }
    </script>
@endpush
