...
@section('title', 'Thông tin đăng ký tour')
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Thông tin đăng ký tour <span class="total-item">(0)</span></h5>
        <div class="btn-group" role="group">
            ...
            <a href="{{ route('user.register_tour.index') }}" class="btn btn-primary me-1">
                Đăng ký tour
            </a>
        </div>
    </div>
...
@push('js')
    <script>
        const routeList = "{{ route('user.register_tour.list') }}";

        filterTable();

        function downFile() {
            var data = $('#form-filter').serialize();
            const url = "{{ route('user.register_tour.list') }}?" + data + "&export=1";
            window.open(url);
        }
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.register_tour.delete') }}");
        }
    </script>
@endpush
