...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Quốc gia <span class="total-item">(0)</span></h5>
        <a href="{{ route('user.country.index') }}" class="btn btn-primary">
            ...
            <table class="table table-bordered table-striped table-sm">
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0 bg-light-primary">
                        ...
                        <th class="text-end w-200px hide-mobile">SL khu vực</th>
                        <th class="text-center w-175px hide-mobile">Ngày tạo</th>
                    </tr>
                </thead>
    ...
    @include('user.pages.country.create')
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('user.country.update') }}" id="form-update" method="POST" enctype="multipart/form-data">
                @csrf
...
@push('js')
    <script>
        const routeList = "{{ route('user.country.list') }}";
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.country.delete') }}");
        }
