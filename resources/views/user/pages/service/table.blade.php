...
@if ($list->count() > 0)
    ...
    @foreach ($list as $item)
        @php
            $status = Services::get_status($item->status);
        @endphp
        <tr id="tr-{{ $item->id }}">
            <td class="text-center">
                ...
                <a data-bs-toggle="tooltip" title="Xem chi tiết"
                    href="{{ route('user.service.detail', ['id' => $item->id]) }}"
            ...
            <td>
                <img src="{{ asset('style/images/blank-sm.jpg') }}" data-src="{{ get_file($item->image) }}" loading="lazy" class="img-fluid w-20px h-20px" alt="Image">
                {{ $item->name }}
            </td>
