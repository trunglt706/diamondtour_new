...
@section('content')
    <div class="d-flex justify-content-between mb-3">
        <h5 class="hide-mobile">Danh mục blog <span class="total-item">(0)</span></h5>
        <div class="btn-group">
            <a href="{{ route('user.blog.index') }}" class="btn btn-secondary">
                Blog
            </a>
            <a href="{{ route('user.blog_group.index') }}" class="btn btn-primary">
                Danh mục
            </a>
        </div>
    </div>
            ...
            <table class="table table-bordered table-striped table-sm">
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0 bg-light-primary">
                        ...
                        <th class="text-center w-175px hide-mobile">SL bài viết</th>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('user.blog_group.update') }}" id="form-update" method="POST"
...
@push('js')
    <script>
        const routeList = "{{ route('user.blog_group.list') }}";
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.blog_group.delete') }}");
        }
