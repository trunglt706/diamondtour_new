...
@section('title', 'Thông tin đăng ký nhận ưu đãi')
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Thông tin đăng ký nhận ưu đãi <span class="total-item">(0)</span></h5>
        <div class="btn-group" role="group">
            <a href="{{ route('user.contact.index') }}" class="btn btn-secondary me-1">
                Liên hệ
            </a>
            <a href="{{ route('user.newllter.index') }}" class="btn btn-secondary me-1">
                Newllter
            </a>
            <a href="{{ route('user.register_promo.index') }}" class="btn btn-primary me-1">
                Nhận ưu đãi
            </a>
            ...
            <table class="table table-bordered table-striped table-sm">
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0 bg-light-primary">
                        ...
                        <th class="text-center w-200px hide-mobile">Mã</th>
                        <th class="text-center">Họ tên</th>
                        <th class="text-center w-250px">Điện thoại</th>
...
@push('js')
    <script>
        const routeList = "{{ route('user.register_promo.list') }}";

        filterTable();

        function downFile() {
            var data = $('#form-filter').serialize();
            const url = "{{ route('user.register_promo.list') }}?" + data + "&export=1";
            window.open(url);
        }
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.register_promo.delete') }}");
        }
    </script>
@endpush
