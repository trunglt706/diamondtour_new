...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Danh mục tour <span class="total-item">(0)</span></h5>
        <div class="btn-group hide-mobile">
            <a href="{{ route('user.tour.index') }}" class="btn btn-secondary me-1">
                Tour
            </a>
            <a href="{{ route('user.tour_group.index') }}" class="btn btn-primary me-1">
                Danh mục
            </a>
            ...
            <table class="table table-bordered table-striped table-sm">
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0 bg-light-primary">
                        ...
                        <th class="text-center w-175px">Thứ tự</th>
                        <th class="text-center w-175px">SL tour</th>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <form action="{{ route('user.tour_group.update') }}" id="form-update" method="POST"
...
@push('js')
    <script>
        const routeList = "{{ route('user.tour_group.list') }}";
        initSelect2('#addModal .filter-country_id', "-- Quốc gia --", 'countries', '#addModal');
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.tour_group.delete') }}");
        }
        ...
        $(document).ready(function() {
            $(document).on("click", ".data-item", function(e) {
                ...
                $.get(url, function(data) {
                    ...
                    initSelect2('#editModal .filter-country_id', "-- Quốc gia --", 'countries',
                        '#editModal');
                })
            })
        })
