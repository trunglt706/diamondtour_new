...
@section('title', 'Album ảnh của tour')
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Album ảnh của tour #{{ $data['tour']->name }}</h5>
        <div class="d-flex justify-content-between">
            <a href="{{ route('user.tour.detail', ['id' => $data['tour']->id]) }}" class="btn btn-secondary me-1">
            ...
            <table class="table table-bordered table-striped table-sm">
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0 bg-light-primary">
                        ...
                        <th class="text-center w-125px hide-mobile">Thứ tự</th>
    ...
    @include('user.pages.tour.library.create')
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('user.tour_library.update') }}" id="form-update" method="POST"
...
@push('js')
    <script>
        const routeList = "{{ route('user.tour_library.list') }}";
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.tour_library.delete') }}");
        }
