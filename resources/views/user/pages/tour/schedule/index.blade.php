...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Lịch trình thuộc tour #{{ $data['tour']->name }} <span class="total-item">(0)</span></h5>
        <a href="{{ route('user.tour.detail', ['id' => $data['tour']->id]) }}" class="btn btn-secondary">
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('user.schedule.update') }}" id="form-update" method="POST"
...
@push('js')
    <script>
        const routeList = "{{ route('user.schedule.list') }}";
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.schedule.delete') }}");
        }
