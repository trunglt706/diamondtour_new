...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="hide-mobile">Danh mục điểm đến <span class="total-item">(0)</span></h5>
        <div class="btn-group">
            <a href="{{ route('user.destination.index') }}" class="btn btn-secondary me-1">
                Điểm đến
            </a>
            <a href="{{ route('user.destination_group.index') }}" class="btn btn-primary me-1">
                Danh mục
            </a>
            ...
            <table class="table table-bordered table-striped table-sm">
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0 bg-light-primary">
                        ...
                        <th class="text-center w-175px hide-mobile">Thứ tự</th>
                        <th class="text-center w-175px hide-mobile">SL điểm đến</th>
    ...
    @include('user.pages.destination.group.create')
    <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('user.destination_group.update') }}" id="form-update" method="POST"
...
@push('js')
    <script>
        const routeList = "{{ route('user.destination_group.list') }}";
        ...
        function confirmDelete(id) {
            deleteData(id, "{{ route('user.destination_group.delete') }}");
        }
